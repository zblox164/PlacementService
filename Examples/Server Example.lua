...
local function checkHitbox(character, object, plot)
	...
	for i = 1, #collisionPoints, 1 do
		...
		if not (not collisionPoints[i]:IsDescendantOf(object) and not collisionPoints[i]:IsDescendantOf(character)) and not collisionPoints[i]:IsDescendantOf() then continue end
